...
uniform sampler2D trashMap;

varying vec2 uv;

const float totStrength = 1.5732;
const float strength = 0.07;
const float offset = 29.4492;
const float falloff = -0.04;
const float rad = -0.009531;
...
//	vec3 fres = normalize((texture2D(rand,uv*offset).xyz*2.0)- vec3(1.0));
//
//	vec4 currentPixelSample = texture2D(normalMap,uv);
//	float currentPixelDepth = currentPixelSample.a;// current fragment coords in screen space
//	vec3 ep = vec3(uv.xy,currentPixelDepth);// get the normal of current fragment
//
//	vec3 norm = currentPixelSample.xyz;
//	float bl =0.0;// adjust for the depth ( not shure if this is good..)
...
//		ray = radD*reflect(pSphere[i],fres);// get the depth of the occluder fragment
//		occluderFragment = texture2D(normalMap,ep.xy+ sign(dot(ray,norm))*ray.xy);// if depthDifference is negative = occluder is behind current fragment
//		depthDifference = currentPixelDepth-occluderFragment.a;// calculate the difference between the normals as a weight

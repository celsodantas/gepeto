...
GLuint loadTexture(const char* fileParam)
{
	// # TODO:: Implementar futuramente uma classe UTIL e 
	// #		incluir essa geração da textura		
	Texture t;
	GLuint textureId;
	
	char file[200];	strcpy(file, fileParam);
	
	bool ok = LoadTGA(&t, file);
	
	if (ok) {
		printf("Sucesso em carregar imagem TGA. \n");
	} else {
		printf("Falha em carregar imagem TGA. \n");
	}
